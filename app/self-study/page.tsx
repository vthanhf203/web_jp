...
export default async function SelfStudyPage() {
  ...
  const [userKanjiStore, vocabStore, readingStore, selfStudyQuizQuestionCount] = await Promise.all([
    ...
    <section className="mx-auto max-w-[1240px] space-y-6 pb-10">
        ...
        <div className="grid gap-6 bg-[#f7fbff] px-6 py-7 lg:grid-cols-[minmax(0,1.1fr)_minmax(340px,0.9fr)] lg:px-8">
          <div>
            ...
            <h1 className="mt-4 max-w-3xl text-4xl font-black leading-tight text-[#111827] md:text-5xl">
              Mỗi kỹ năng một màn hình riêng
            </h1>
            <p className="mt-4 max-w-2xl text-base leading-7 text-[#667085]">
              Chọn đúng mục bạn muốn luyện để màn hình học rộng hơn, ít rối hơn và dễ tập trung hơn.
            </p>
          </div>
          <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-1">
            ...
            <div className="rounded-2xl border border-[#d8e2ee] bg-white px-4 py-3">
              <p className="text-xs font-black uppercase tracking-[0.14em] text-[#64748b]">Từ vựng</p>
              <p className="mt-1 text-2xl font-black text-[#111827]">{totalVocabItems}</p>
            </div>
            <div className="rounded-2xl border border-[#d8e2ee] bg-white px-4 py-3">
              <p className="text-xs font-black uppercase tracking-[0.14em] text-[#64748b]">Bài đọc</p>
      ...
      <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
        <StudyCard
          ...
          title="Từ vựng & Kanji"
          description="Import kho riêng, chọn bài học, luyện flashcard và quiz Kanji cá nhân."
          stat={`${vocabStore.lessons.length} bài từ vựng · ${userKanjiStore.items.length} Kanji`}
        ...
        <StudyCard
          ...
          title="Đọc văn bản"
          description="Luyện đọc bài tiếng Nhật theo cấp độ, kèm từ mới, dịch nghĩa và câu hỏi nhanh."
          stat={`${readingStore.items.length} bài đọc · ${totalReadingWords} từ mới`}
        ...
        <StudyCard
          ...
          title="Quiz JSON"
          description="Ôn các bộ câu hỏi đã import, chọn bộ riêng và làm bài trong một màn hình thoáng hơn."
          stat={`${selfStudyQuizQuestionCount} câu hỏi`}
        ...
        <StudyCard
          ...
          description="Xem lại toàn bộ Kanji đã import, từ liên quan và trạng thái dữ liệu cá nhân."
          stat={`${userKanjiStore.items.length} chữ đã lưu`}
      ...
      <div className="rounded-[24px] border border-[#d8e2ee] bg-white p-5 shadow-[0_18px_42px_rgba(20,47,80,0.08)]">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-black text-[#111827]">Luồng tự học đã được tách trang</h2>
              <p className="mt-1 text-sm text-[#667085]">
                Các form import nặng nằm trong từng trang con để trang tổng không còn bị chật.
              </p>
            </div>
          </div>
          <Link
